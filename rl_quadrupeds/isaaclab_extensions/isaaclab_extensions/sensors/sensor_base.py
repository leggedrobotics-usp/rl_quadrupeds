...
class SensorBase(ABC):
    ...
    def __init__(self, cfg: SensorBaseCfg):
        ...
        # initialize the ros publisher if enabled
        self._initialize_ros_publisher()
    ...
    def _update_outdated_buffers(self):
        ...
        if len(outdated_env_ids) > 0:
            ...
            # if ROS publisher is enabled, publish the data
            if getattr(self.cfg, 'ros_publisher_cfg', False):
                self._ros_publisher.publish(self._data, outdated_env_ids)

    def _initialize_ros_publisher(self):
        """Initializes the ROS publisher for the sensor data."""
        if getattr(self.cfg, 'ros_publisher_cfg', False):
            import rclpy
            self._ros_publisher = self.cfg.ros_publisher_cfg.node_class(self.cfg)
            rclpy.spin_once(self._ros_publisher, timeout_sec=0)