...
class Go1LocomotionCurriculumCfg(ManagerTermBase):
    """Synchronized curriculum for GO1 locomotion with per-stage reward weighting.
    Works with normalized reward scaling:
        • Penalties in [-1, 0]
        • Bonuses in [0, 1]
    """
    ...
    def __init__(self, cfg: SceneEntityCfg, env: "ManagerBasedRLEnv"):
        ...
        self.STAGES = [
            ...
            # ---------------------------------------------------------
            # Stage 1: Stable posture and torque control
            # ---------------------------------------------------------
            dict(
                ...
                reward_weights={
                    ...
                    "joint_pos_limits_penalty": 0.8,
                    "joint_torques_l2": 0.4,
                    "joint_acc_penalty": 0.3,
                    "action_rate_penalty": 0.2,
                    "applied_torque_limits_bonus": 0.6,
                    "is_terminated": 1.0,
                },
            ),

            # ---------------------------------------------------------
            # Stage 2: Controlled height & posture
            # ---------------------------------------------------------
            dict(
                ...
                reward_weights={
                    ...
                    "penalize_foot_deviation_from_default": 0.6,
                    "joint_torques_l2": 0.4,
            ...
            # ---------------------------------------------------------
            # Stage 3: Small movement commands (gentle motion)
            # ---------------------------------------------------------
            dict(
            ...
            # ---------------------------------------------------------
            # Stage 4: Introduce trotting gait pattern
            # ---------------------------------------------------------
            dict(
                ...
                reward_weights={
                    "track_lin_vel_xy_exp": 1.0,
                    "track_ang_vel_z_exp": 0.8,
                    "trot_gait_positive": 0.6,
                    "trot_gait_negative": 0.3,
                    "trot_diag_joint_symmetry": 0.3,
                    "diag_motion_balance": 0.2,
            ...
            # ---------------------------------------------------------
            # Stage 5: Full dynamic locomotion
            # ---------------------------------------------------------
            dict(
                ...
                reward_weights={
                    ...
                    "diag_motion_balance": 0.2,
                    "track_footswing_height": 0.5,
                    "track_joint_positions_bonus": 0.2,
                    "penalize_hip_movement": 0.3,
                    "track_feet_slip": 0.3,
                    "staying_still_penalty": 0.3,
                    "flat_orientation_bonus": 0.6,
                    "track_base_height_l2_cmd_no_sensor": 0.8,
                    "joint_torques_l2": 0.4,
                    "joint_acc_penalty": 0.2,
    ...
    def __call__(self, env: "ManagerBasedRLEnv", env_ids: Sequence[int]):
        ...
        if (stage_step - self.warmup_steps) % (self.num_steps_per_env - 1) == 0:
            ...
            if stage_step > self.warmup_steps:
                ...
                # Detect periodic variance to prevent false progression
                if len(self.reward_window) >= self.reward_window.maxlen // 2:
                    ...
                    if len(ac_middle) > 50:
                        max_ac = ac_middle[10:50].max().item()
                        if max_ac > 0.8:
                            self.rel_var *= 0.05
    ...
    def _apply_stage(self, env: "ManagerBasedRLEnv", stage_idx: int):
        ...
        # Apply per-stage custom reward weights
